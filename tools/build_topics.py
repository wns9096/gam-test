...
# ★ 「한 해에 몇 번」이 아니라 「몇 시간에 한 번」으로 묻는다. 같은 사실인데
#   뒤집어 물으면 크기가 전혀 다르게 느껴진다 — 이 앱이 재려는 것이 그것이다.
#   날수는 달력에서 센다(윤년이 7번 들어 있다). 25×365 로 어림하지 않는다.
SPAN_DAYS = (dt.date(Q_YEARS[-1] + 1, 1, 1) - dt.date(Q_YEARS[0], 1, 1)).days
GAP_HOURS = SPAN_DAYS * 24 / len(QUAKE)

# 깊이. 70km 를 경계로 얕은 지진과 그 아래를 가르는 것이 보통이다.
DEEP = [x for x in QUAKE if x["depth"] >= 70]
DEEP_PCT = len(DEEP) / len(QUAKE) * 100
DEEPEST = max(x["depth"] for x in QUAKE)
# ★ 원본을 열어 본 김에 깊이 값의 분포도 세었다. 28%가 «정확히 10.0km» 다 —
#   잰 값이 아니라 정하기 어려울 때 그 값에 고정해 놓고 계산한 것이다.
FIXED10 = sum(1 for x in QUAKE if x["depth"] == 10.0) / len(QUAKE) * 100
FIXED33 = sum(1 for x in QUAKE if x["depth"] == 33.0) / len(QUAKE) * 100
...
topic(
    ...
    [
        slider("z1", "전 세계 어딘가에서 규모 5.5 이상 지진은 "
                     "평균 몇 시간에 한 번 일어날까요?",
               GAP_HOURS, "시간", 0, 72, 2,
               f"{Q_YEARS[0]}-01-01~{Q_YEARS[-1]}-12-31 은 {SPAN_DAYS:,}일 "
               f"= {SPAN_DAYS * 24:,}시간. 이것을 지진 {len(QUAKE):,}건으로 "
               f"나누면 {GAP_HOURS:.1f}시간에 한 번입니다. 목록에 섞여 있던 "
               f"지진 아닌 {len(NOTQ)}건은 빼고 세었습니다",
               "뉴스에 나온 지진만 기억에 남아서 「가끔」이라고 느낍니다. "
               "규모 5.5면 큰 지진인데, 사람이 안 사는 바다에서 나면 "
               "아무도 모릅니다."),

        slider("z2", "규모 5.5 이상 지진 가운데 땅속 70km 보다 "
                     "깊은 곳에서 난 것은 몇 %일까요?",
               DEEP_PCT, "%", 0, 60, 2,
               f"{len(QUAKE):,}건 중 깊이 70km 이상이 {len(DEEP):,}건 "
               f"= {DEEP_PCT:.1f}%. 가장 깊은 것은 {DEEPEST:.0f}km 입니다 "
               f"(70km 를 경계로 얕은 지진과 그 아래를 가르는 것이 보통입니다)",
               "지진은 발밑 가까이에서 나는 줄 알지만 다섯에 하나 가까이가 "
               "70km 아래이고, 가장 깊은 것은 수백 km 아래입니다. "
               "깊으면 흔들림이 넓게 퍼지는 대신 약해집니다."),
    ...
    caveat=(
        ...
        "있습니다).<br>"
        f"깊이는 더 조심해야 합니다. 이 목록에서 <b>{FIXED10:.1f}%가 깊이 "
        f"「정확히 10.0km」</b> 이고 {FIXED33:.1f}%가 「정확히 33.0km」 입니다 — "
        "그렇게 잰 것이 아니라 깊이를 정하기 어려울 때 그 값에 고정해 놓고 "
        "계산한 것입니다.<br>"
    ...
    notes=[
        ...
        f"날수는 한 번 빼는 대신 <b>해마다 세어 더했고</b>(윤년 7일이 "
        f"들어 있습니다), 깊이는 깊은 쪽 대신 <b>얕은 쪽을 세어 뺐습니다.</b> "
        f"생성한 코드를 불러 쓰면 같은 식을 두 번 돌리는 것이라 대조가 "
        f"아니므로 데이터 파일만 다시 읽습니다.",
    ],
)
